...
function UpdateUser() {
  ...
  const getUserDetails = async () => {
    let user = await fetch(`http://localhost:3003/users/${params.id}`);
    user = await user.json();
    setUser(user);
    // setUser mul data yete
    console.log(user);
  };
  ...
  const updateUserDetails = async () => {
    ...
    // navigate("/all");
    console.log(user);
    let result = await fetch(`http://localhost:3003/users/${params.id}`, {
      method: "put",
      body: JSON.stringify({ name, address, hobbies, birtdate, female, male, college }),
      headers:{
  ...
  return (
    ...
    <FormControlLabel
      ...
      >
        ...
        <MenuItem value="Middlesex Community College">
          Middlesex Community College
        </MenuItem>
        <MenuItem value="Middlesex County College">
          Middlesex County College
        </MenuItem>
        <MenuItem value="Middlebury College">Middlebury College</MenuItem>
        <MenuItem value="Middle East University">
          Middle East University
        </MenuItem>
        <MenuItem value="Middle East Technical University">
          Middle East Technical University
        </MenuItem>
        <MenuItem value="Middle Tennessee State University">
          Middle Tennessee State University
        </MenuItem>
        <MenuItem value="American University of Middle East">
